...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    api.get('/rail-survey?offset=0', null).then((resp) => {
      resp.json().then((json) => {
  ...
  const filterData = useMemo(() => {
    ...
    let raw = rawData
    try {
      rtnData = handleFetchResult(rawData)
      for (const key in queryFilter) {
        if (['area', 'zone', 'station'].includes(key)) {
          if (key === 'station') {
            // for (const key2 in queryFilter[key]) {
            //   raw = rawData.filter(rData => rData.generalSurvey[key] === queryFilter[key])
            //   rtnData = handleFetchResult(raw)
            // }
          } else {
            if (queryFilter[key]) {
              raw = rawData.filter(rData => rData.generalSurvey[key] === queryFilter[key])
